...
> - 实际开发分支 = `dev`（融合线已全部合入 `dev`，替代下方全部 `feat/dsh-integration` 引用；`main` = 融合前旧工程存档）。
...
> 2026-08-20 更新：主线分支名已改为 `dev`（`refactor/integrate-into-dsh` 已合并回 `dev` 并删除；下方 `feat/dsh-integration` 均指同一开发线，合并/回退策略不变）；DSH 本体已 submodule 化，升级走 `scripts/upgrade-dsh.sh`。

- 本融合项目的全部开发在**独立新分支**上进行，**不直接动 `dev`/`main`**：
  - 主线开发分支：`dev`（融合工作线已全部合入）
  - 大型子阶段可再开子分支（如 `feat/dsh-llm-gateway`、`feat/dsh-research-paper`），合回 `dev`
- `main` 保留融合前旧工程（存档）；如 GitHub 默认分支仍是 `main`，建议改为 `dev`（`gh repo edit uaenamyf/AI-research-manager-OS --default-branch dev`）。
- 阶段性成果（每完成一个 bundle/里程碑）合入 `dev` 并打 tag（如 `dsh-m0-verified`、`dsh-p1-gateway`），便于回滚定位。
- 每次合并前跑对应服务测试（`make test-*`）；跨服务契约变更遵循根 `AGENTS.md` §5。
- 融合工作线已全部合入 `dev`（原 `refactor/integrate-into-dsh` 已合并删除）。
