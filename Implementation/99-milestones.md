...
> 本文档 Sprint 1-3 为 **legacy 三服务原路线**（Next.js + Spring Boot + FastAPI）的任务拆解，全部任务已完成；其中前端（Next.js :3000）已随融合下线（可回退）。融合后的里程碑以根 `plan.md`（融合方案）+ `dsh-plugins/README.md`（bundle 实现记录，含各 Phase 验证）为权威，进度如下：
...
| Phase 4 | 前端 UI 11 包（ui-research-hello 探针 + 10 业务节点）+ Next.js 下线 | ✅ 11 包注入 boot（49 条目）；Next.js :3000 已停（可回退） |
