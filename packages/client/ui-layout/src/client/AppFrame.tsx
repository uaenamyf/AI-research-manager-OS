...
export function AppFrame({
  ...
  // The details drag handle anchors to the frame's *content-box* right edge,
  // not the window: a host plugin may reserve room on the frame's right
  // (padding-right, e.g. a fixed activity rail), which shifts the grid tracks
  // left of the viewport edge. clientWidth is the padding box; subtract the
  // right padding to reach the content box.
  const frameEl = frameRef.current
  const frameRight = frameEl
    ? frameEl.clientWidth - (parseInt(getComputedStyle(frameEl).paddingRight, 10) || 0)
    : viewport
  ...
  return (
    ...
    >
      ...
      {cols.details > 0 && <DragHandle side="details" left={frameRight - cols.details} onStart={onDetailsStart} onDrag={onDetailsDrag} onEnd={onDragEnd} />}
    </div>
  )
}
