...
> ResearchOS 已融入 DeepSeek Harness（DSH）。
> **2026-08-22 更新**：单仓库化 —— fork `uaenamyf/dsh-researchOS` 即唯一仓库（无 submodule），main = 自定义版本（upstream = `deepseek-ai/deepseek-harness`）；每次 DSH 升级运行 `packages/researchos/scripts/upgrade-dsh.sh <tag>`。

### 0.1 当前形态

- **DSH 单实例驻 `127.0.0.1:3080`**（`scripts/dsh-gateway.sh` 启动，自动注入 `.env` 的 key / JWT / 数据目录等环境变量），单个 DSH 进程承载：
  - **统一 LLM 网关 `research-llm-gateway`**：OpenAI 兼容直连代理，`POST /v1/chat/completions` + `POST /v1/embeddings`；上游 key/模型单点收口（chat `ark-code-latest` / embedding `doubao-embedding-vision`，来源 `.env`）。
...
- 改动 DSH 融合包（`packages/researchos/`）必须遵循包结构规范：`package.json` 声明 `dsh.bundle`（插件）或 `dsh.client`（客户端 UI 包）、`cordis.patch.yml` 自挂载（`- insert:` 行）、`index.js` `export function apply(ctx)`（Cordis 插件形态）。
...
- 融合相关新工作（bundle / MCP / 网关 / UI 包）以本章节 + `packages/researchos/` 为准。
...
- **开发 agent 角色**：DSH 前端工程师（改动 `packages/researchos/ui/ui-research-*` 客户端包，遵循 `packages/client/AGENTS.md` 的 slot / props 规范）。
- **职责**：
...
- 契约变更先改文档再改代码（契约文档见本包 README 与代码注释）。
