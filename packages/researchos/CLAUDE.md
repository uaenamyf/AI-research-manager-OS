...
> - 融合包约束见 `packages/researchos/` 与 DSH `packages/client/AGENTS.md`。
...
- **语言与版本**：融合包以 TypeScript/JavaScript 为主（DSH `packages/researchos/`，bundle 为 Cordis 插件形态、UI 包为 React 客户端包）；DSH 本体见仓库根 `CLAUDE.md`。
...
- 新增依赖必须更新对应 `package.json`（DSH 融合包在 `packages/researchos/`，版本锁定）。
- DSH 本体依赖（pnpm workspace）见仓库根 `package.json` / `pnpm-workspace.yaml`。
- **DSH 升级（fork 单仓库）**：本仓库是 fork（main = 自定义版本，upstream = `deepseek-ai/deepseek-harness`）。每次上游发版运行 `packages/researchos/scripts/upgrade-dsh.sh <tag>`（如 `rc.9`）：
  - fetch upstream tag → 从 main 建 `upgrade/<tag>` 分支 → merge（冲突预期仅限修改过的上游文件）→ `pnpm install` + `build:lib:host/client` + `build:web` → `pnpm vitest run packages/client/ui-layout` → `--push` 更新 fork main。
  - 注意：仓库内 `node_modules` / `lib` / `dist` / `vendor/*/lib` 是构建产物（git 不跟踪），升级 merge 后如缺失需重建或从备份恢复，不能只靠 git 内容。

# 8. 测试要求

- 融合包（bundle / UI 包 / ai-worker）：改动按 DSH `packages/client/AGENTS.md` 与 `packages/researchos/` 的规范验证；LLM 相关改动保持可 mock、CI 不消耗真实 token。
