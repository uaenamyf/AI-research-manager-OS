...
// One-time upload tokens are held in-process (like the backend's map). Reads
// fall back to proxying the legacy backend (/api/files/{key} with
// X-Internal-Token) so already-stored PDFs keep serving during the dual phase.
//
...
//   GET    /research-file/files/{key...}        full or Range download    -> local, else legacy proxy    (JWT or X-Internal-Token)
...
//   RESEARCH_BACKEND_URL         (legacy proxy fallback, default http://127.0.0.1:8080)
...
const BACKEND_URL = (process.env.RESEARCH_BACKEND_URL || 'http://127.0.0.1:8080').replace(/\/+$/, '')
...
export function apply(ctx) {
  ...
  ctx.logger.info(
    `[research-file] loaded — local file storage at ${UPLOAD_DIR}, legacy proxy ${BACKEND_URL}`,
  )
    ...
    handler: async (req, res) => {
      ...
      if (seg[0] === 'files' && seg[1]) {
        ...
        if (method === 'GET') {
          ...
          // Legacy fallback: proxy the backend (already-stored PDFs live in its storage).
          try {
