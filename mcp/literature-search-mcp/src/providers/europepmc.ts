...
export function parseEuropePmc(data: EuropePmcResponse): ProviderPaper[] {
  return (data.resultList?.result ?? []).map((record) => {
    ...
    const authors = record.authorList?.author?.map(
      (author) => author.fullName ?? [author.firstName, author.lastName].filter(Boolean).join(" "),
    ) ?? (record.authorString ? record.authorString.split(/,\s*|;\s*/) : [])
...
function query(input: SearchInput): string {
  const parts = [`(${input.query})`]
  if (input.year_from !== undefined) parts.push(`FIRST_PDATE:[${input.year_from}-01-01 TO 3000-12-31]`)
  if (input.year_to !== undefined) parts.push(`FIRST_PDATE:[1000-01-01 TO ${input.year_to}-12-31]`)
  if (input.open_access) parts.push("OPEN_ACCESS:Y")
  return parts.join(" AND ")
}

export const europepmc: LiteratureProvider = {
  id: "europepmc",
  name: "Europe PMC",
  ...
  async search(input, context) {
    ...
    const data = await context.http.getJson<EuropePmcResponse>(`${BASE}?${params}`, { signal: context.signal })
