...
export function parseArxivXml(xml) {
    ...
    const entries = toArray(root.feed?.entry);
    return entries
        .map((entry) => {
        const record = entry;
        ...
        const links = toArray(record.link).filter((link) => Boolean(link && typeof link === "object"));
        const pdf = links.find((link) => String(link["@title"] ?? "").toLowerCase() === "pdf" || String(link["@type"] ?? "").toLowerCase() === "application/pdf")?.["@href"];
        const category = toArray(record.primary_category)[0];
        return makePaper({
            source: "arxiv",
            ...
            authors: toArray(record.author).map((author) => stringValue(author?.name)),
...
export const arxiv = {
    ...
    async search(input, context) {
        ...
        if (!/<feed[\s>]/i.test(xml))
            throw new Error("arXiv returned an invalid Atom response");
