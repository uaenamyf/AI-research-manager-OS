...
import type { LiteratureProvider, ProviderContext, ProviderPaper, SearchInput } from "../types.js"
...
export function parseBiorxiv(data: PreprintResponse, fallbackServer: PreprintServer): ProviderPaper[] {
...
function score(paper: ProviderPaper, queryTerms: string[]): number {
...
async function byDoi(server: PreprintServer, doi: string, context: ProviderContext): Promise<ProviderPaper[]> {
  const data = await context.http.getJson<PreprintResponse>(`${BASE}/${server}/${doi}`, {
...
async function recent(server: PreprintServer, pool: number, context: ProviderContext): Promise<ProviderPaper[]> {
  const papers: ProviderPaper[] = []
  let cursor = 0
  for (let page = 0; page < MAX_PAGES_PER_SERVER && papers.length < pool; page++) {
    let data: PreprintResponse
    try {
      data = await context.http.getJson<PreprintResponse>(`${BASE}/${server}/${pool}/${cursor}`, {
        signal: context.signal,
        rateLimit: { maxConcurrent: 2 },
      })
    } catch (error) {
    ...
    if (count === 0 || pagePapers.length === 0 || (total !== undefined && cursor + count >= total)) break
    cursor += count
  }
  return papers.slice(0, pool)
}

export const biorxiv: LiteratureProvider = {
  id: "biorxiv",
  ...
  async search(input, context) {
    ...
    const fulfilled: ProviderPaper[][] = []
    for (let index = 0; index < settled.length; index++) {
      const outcome = settled[index]
      const server = SERVERS[index]
      if (!outcome || !server) continue
      if (outcome.status === "fulfilled") fulfilled.push(outcome.value)
      else context.warnings?.push(`${server} backend failed: ${sanitizedError(outcome.reason).type}`)
    }
    ...
    if (doi) return papers.slice(0, input.limit)
