...
import type { LiteratureProvider, SearchInput, SearchResponse, SourceId, SourceStatus } from "./types.js"
...
export class LiteratureSearchService {
  ...
  async search(raw: Partial<SearchInput> & Pick<SearchInput, "query">, signal?: AbortSignal): Promise<SearchResponse> {
    const input = normalizeInput(raw)
    const wanted = new Set<SourceId>(input.sources ?? SOURCE_ORDER)
    ...
    const outcomes = await Promise.all(
      selected.map(async (provider) => {
        ...
        try {
          const papers = applyCommonFilters(await provider.search(input, { http: this.http, signal, warnings }), input).slice(
            0,
            input.limit,
          )
    ...
    const results = deduplicateAndFuse(
      outcomes.map((outcome) => ({ source: outcome.source, papers: outcome.papers })),
      input.limit,
    )
...
export function normalizeInput(raw: Partial<SearchInput> & Pick<SearchInput, "query">): SearchInput {
  ...
  if (requested && (!sources || sources.length === 0)) throw new Error("sources must include at least one supported source")
  return {
    query,
    limit,
    sources,
