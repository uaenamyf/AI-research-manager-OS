#!/usr/bin/env bash
# 一键升级 DSH submodule（方案 A：submodule + fork custom main）
#
# 背景（AGENTS.md §0）：
#   deepseek-harness-master 是 git submodule，远端 = fork
#   (uaenamyf/dsh-researchOS)，其 main 分支 = 自定义版本
#   （rc.8 merge + researchos 自定义 + 修复 15 个上游文件）。
#   researchos 包是纯新增（零冲突），冲突只应出现在修改过的上游文件。
#
# 用法:
#   scripts/upgrade-dsh.sh <tag> [--push]
#     <tag>    上游 tag：dsh-v0.1.0-rc.9（支持简写 rc.9）
#     --push   自动把 fork main + 父仓库 submodule 指针 push 到 GitHub
#
# 流程:
...
#   6. 跑 ui-layout 测试
#   7. --push: 合并回 main -> push fork -> 父仓库更新 submodule 指针 -> push
#
# 升级完成后记得重启 gateway: bash scripts/dsh-gateway.sh restart

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DSH="$REPO_ROOT/deepseek-harness-master"
UPSTREAM_URL="https://github.com/deepseek-ai/deepseek-harness.git"
TAG_PREFIX="dsh-v0.1.0-"

TAG="${1:-}"
[ -n "$TAG" ] || { echo "用法: scripts/upgrade-dsh.sh <tag> [--push]  (tag 如 rc.9 / dsh-v0.1.0-rc.9)"; exit 1; }
PUSH_MODE="${2:-}"
case "$TAG" in
  rc*) TAG="${TAG_PREFIX}${TAG}" ;;
esac

[ -d "$DSH/.git" ] || { echo "错误: submodule 不存在于 $DSH"; exit 1; }
...
if [ "$TAG" = "--finish" ]; then
  ...
  if ! git merge "$TAG" -m "merge upstream $TAG"; then
    ...
    echo "   # 然后继续剩余步骤："
    echo "   bash $REPO_ROOT/scripts/upgrade-dsh.sh --finish $TAG ${PUSH_MODE:-}"
    exit 1
  fi
fi
...
if [ "$PUSH_MODE" = "--push" ]; then
  echo "==> [6/6] push fork main + 父仓库 submodule 指针 ..."
  git checkout main
  git merge "$BR" --ff-only
  git push origin main
  git push origin "$BR" || true   # 保留升级分支（失败不阻塞）
  cd "$REPO_ROOT"
  git submodule update --remote deepseek-harness-master
  git add deepseek-harness-master
  git commit -m "chore: bump DSH submodule to $TAG" || true
  git push
  echo "==> 完成！重启 gateway 验证: bash scripts/dsh-gateway.sh restart"
else
  echo ""
  echo "==> 升级完成（未 push）。验证无误后执行："
  echo "   bash $REPO_ROOT/scripts/upgrade-dsh.sh --finish $TAG --push"
  echo "   或手动: cd $DSH && git checkout main && git merge $BR --ff-only && git push origin main"
  echo "   + 父仓库: git submodule update --remote deepseek-harness-master && git add deepseek-harness-master && git commit && git push"
  echo "   + 重启 gateway: bash scripts/dsh-gateway.sh restart"
fi
